...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 200000) {
      if (Aku > Kamu) {
        conn.reply(m.chat, `Kamu Tertangkap Setelah Kamu Korupsi Dana Rakyat, Dan Kamu Harus Membayar Denda Sebesar 250k Eris Agar Bisa Terbebas Dari Penjara`, m)
        user.eris -= 250000
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.eris += 100000
        conn.reply(m.chat, `Kamu Berhasil Korupsi Dana Rakyat,  Dan Kamu Mendapatkan 100k Eris`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else {
