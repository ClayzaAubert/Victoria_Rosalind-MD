...
let handler = async (m, { conn, args, usedPrefix }) => {
    ...
    if (args.length < 1) {
        return conn.reply(m.chat, `Silakan masukkan jumlah Eris yang ingin Anda pertaruhkan.\nContoh: *${usedPrefix}casino 1000*`, m)
    }
    try {
        ...
        let count = args[0]
        count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[m.sender].eris / buatall) : parseInt(count) : args[0] ? parseInt(args[0]) : 1
...
*Menang*
Kamu Mendapatkan ${count * 2} eris`.trim()
    conn.reply(m.chat, caption, m, { mentions: conn.parseMention(caption) })
    } else {
    let caption = `*C A S I N O*
...
Kamu Mendapatkan ${count * 1}`.trim()
    conn.reply(m.chat, caption, m, { mentions: conn.parseMention(caption) })
        ...
        } else conn.reply(m.chat, `Eris Kamu Tidak Mencukupi Untuk Casino Silahkan!`.trim(), m)
    } catch (e) {
        console.log(e)
        m.reply("Error!!")
    } finally {
